...
public class TrailEffect {
    ...
    public static void create(Entity entity, TrailProvider provider) {
        create(new TrailEffect(entity, provider));
    }
    ...
    public static void removeByUuid(UUID uuid) {
        TRAILS.removeIf(x -> uuid.equals(x.provider.getOwnerUuid()));
    }
    ...
    public static void tick(MinecraftClient client) {
        if (client.world != null) {
            ...
            for (Entity entity : client.world.getEntities())
                if (entity instanceof TrailProvider provider && TRAILS.stream().noneMatch(effect1 -> entity.getUuid().equals(effect1.provider.getOwnerUuid())))
    ...
    public void worldTick(MinecraftClient client) {
        if (client.world == null || this.provider.shouldRemove() || this.effect.getLength() <= 0 || this.provider.getCurrentPos().distanceTo(MinecraftClient.getInstance().gameRenderer.getCamera().getPos()) > this.provider.maxDistance())
            this.shouldRemove = true;
        else this.provider.updateTrail(this.effect);
    }
